 ...
 */
async function loadCredentials(agent: any, holderDid: string): Promise<any[]> {
  try {
...
async function testDIDCommVPFlow() {
  ...
  const { agent: agentA, dbConnection: dbA } = await createAgentWithDatabase(
    DB_PATH_NF_A,
    DB_ENCRYPTION_KEY_A
  );
  console.log('   ✅ Agent NF-A created');

  const { agent: agentB, dbConnection: dbB } = await createAgentWithDatabase(
    DB_PATH_NF_B,
    DB_ENCRYPTION_KEY_B
  );
  ...
  try {
    ...
    const result = await performMutualAuthentication(
      agentA,
      agentB,
      DID_NF_A,
      DID_NF_B,
    ...
    if (result.nfAAuthenticated && result.nfBAuthenticated) {
      ...
      (wrapperA as any).contexts.set(DID_NF_B, mockContext);

      const serviceRequest = await wrapperA.sendServiceRequest(
        DID_NF_B,
        'credential-issuance',
        'issue-credential',
        { credentialType: 'VerifiableCredential', subject: 'test' }
      );
