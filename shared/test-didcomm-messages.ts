...
function printMessage(title: string, message: DIDCommVPMessage) {
  console.log(`\n${title}`);
...
const vpWithPD = createVPWithPD(
  DID_NF_B,
  DID_NF_A,
  mockVP_B,
  PRESENTATION_DEFINITION_B,
  'Here is my VP and my PD for you'
);
...
const vpResponse = createVPResponse(
  DID_NF_A,
  DID_NF_B,
  mockVP_A,
  'Here is my VP'
...
const authConfirmation = createAuthConfirmation(
  DID_NF_B,
  DID_NF_A,
  'OK',
  sessionToken,
...
const serviceRequest = createServiceRequest(
  ...
  'credential-issuance',
  'issue-credential',
  { credentialType: 'VerifiableCredential', subject: 'test-subject' },
  sessionToken
);
