 ...
 * 4. Sendet an NF_B
 */
export async function initiateServiceRequest(agent, nfBDID, serviceRequest) {
 ...
 * 7. Sende VP_A zurück
 */
export async function handlePresentationFromNFB(agent, packedMessage) {
    ...
    const vpB = message.data.attachments.find((a) => a.id === 'presentation')?.data.json;
    const pdB = message.data.attachments.find((a) => a.id === 'sdr-request')?.data.json;
 ...
 */
export async function handleAuthorizedConfirmation(agent, packedMessage) {
 ...
 */
export async function sendServiceRequest(agent, sessionId, serviceRequest) {
    const session = sessions.get(sessionId);
 ...
 */
export async function handleInitialRequestFromNFA(agent, packedMessage) {
    ...
    const sessionId = message.id;
    const sdrFromA = message.data.attachments.find((a) => a.id === 'sdr-request')?.data.json;
    const serviceRequest = message.data.attachments.find((a) => a.id === 'service-request')?.data.json;
 ...
 * 5. Sende Bestätigung
 */
export async function handlePresentationFromNFA(agent, packedMessage) {
    ...
    const vpA = message.data.attachments.find((a) => a.id === 'presentation')?.data.json;
 ...
 */
export async function handleServiceRequest(agent, packedMessage, nfBusinessLogic) {
 ...
 */
export async function handleIncomingDIDCommMessage(agent, packedMessage, isNFA, nfBusinessLogic) {
 ...
 */
export function cleanupExpiredSessions(maxAge = 3600000) {
    const now = new Date();
