...
// Helper Functions
// ============================================================================
async function createAgentForCluster(config) {
    ...
    const agent = createAgent({
        plugins: [
            new KeyManager({
                ...
                kms: {
                    local: new KeyManagementSystem(new PrivateKeyStore(dbConnection, new SecretBox(config.secretKey)))
                }
            }),
            new DIDManager({
...
async function createX25519KeyForDID(agent, did) {
    console.log(`\n📝 Processing: ${did}`);
    ...
    const keyAgreementKeys = didDoc.keys.filter((k) => k.type === 'X25519');
...
function generateDIDDocumentJSON(did, publicKeyHex, kid) {
...
async function main() {
    ...
    for (const result of allResults) {
        if (result.keyAgreementKeys.length > 0) {
            ...
            const jsonEntry = generateDIDDocumentJSON(result.did, latestKey.publicKeyHex, latestKey.kid);
            console.log(`\n📄 ${jsonEntry.filePath}`);
            console.log('─'.repeat(80));
