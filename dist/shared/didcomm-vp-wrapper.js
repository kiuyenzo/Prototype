...
import { createVPAuthRequest, createVPWithPD, createVPResponse, createAuthConfirmation, createServiceRequest, createErrorMessage } from './didcomm-messages.js';
/**
...
export class DIDCommVPWrapper {
    ...
    async initiateVPAuthRequest(ourDid, theirDid, presentationDefinition) {
        ...
        const message = createVPAuthRequest(ourDid, theirDid, presentationDefinition, 'Please provide a Verifiable Presentation matching this definition');
    ...
    async handleVPAuthRequest(message, ourDid, credentials, ourPresentationDefinition) {
        ...
        try {
            ...
            const vp = await createVPFromPD(this.agent, ourDid, credentials, message.body.presentation_definition);
            context.ourVP = vp;
            // Create response message with our VP and our PD
            const responseMessage = createVPWithPD(ourDid, message.from, vp, ourPresentationDefinition, 'Here is my VP matching your PD, and my PD for you to satisfy');
            ...
            const errorMessage = createErrorMessage(ourDid, message.from, 'vp_creation_failed', `Failed to create VP: ${error.message}`);
    ...
    async handleVPWithPD(message, credentials, ourDid, ourPresentationDefinition) {
        ...
        try {
            ...
            const verificationResult = await verifyVPAgainstPD(this.agent, message.body.verifiable_presentation, context.ourPresentationDefinition);
            ...
            const ourVP = await createVPFromPD(this.agent, context.ourDid, credentials, message.body.presentation_definition);
            context.ourVP = ourVP;
            // Create response message
            const responseMessage = createVPResponse(context.ourDid, message.from, ourVP, 'Here is my VP matching your PD');
            context.messageLog.push(responseMessage);
            this.messageQueue.push(responseMessage);
            ...
            const errorMessage = createErrorMessage(context.ourDid, message.from, 'vp_verification_failed', `Failed to verify or create VP: ${error.message}`);
            this.messageQueue.push(errorMessage);
            throw error;
        }
    }
    ...
    async handleVPResponse(message) {
        ...
        try {
            ...
            const confirmationMessage = createAuthConfirmation(context.ourDid, message.from, 'OK', sessionToken, 'Mutual authentication successful');
            ...
            // Send rejection
            const rejectionMessage = createAuthConfirmation(context.ourDid, message.from, 'REJECTED', undefined, `VP verification failed: ${error.message}`);
    ...
    async sendServiceRequest(theirDid, service, action, params) {
        ...
        const requestMessage = createServiceRequest(context.ourDid, theirDid, service, action, params, context.sessionToken);
 ...
 */
export async function performMutualAuthentication(agentA, agentB, didA, didB, credentialsA, credentialsB, pdA, pdB) {
