...
kubectl config use-context kind-cluster-a > /dev/null 2>&1
HEALTH_A=$(kubectl exec -n nf-a-namespace $POD_A -c veramo-nf-a -- wget -q -O- http://172.23.0.2:31829/health 2>&1)
echo "Cluster-A Health: $HEALTH_A"

kubectl config use-context kind-cluster-b > /dev/null 2>&1
HEALTH_B=$(kubectl exec -n nf-b-namespace $POD_B -c veramo-nf-b -- wget -q -O- http://172.23.0.3:30132/health 2>&1)
