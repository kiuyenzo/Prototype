...
async function addX25519Key(did: string, dbPath: string, encryptionKey: string, didDocPath: string, name: string) {
  console.log(`\n${'='.repeat(80)}`);
  ...
  try {
    ...
    const x25519Keys = identifier.keys.filter((k: any) => k.type === 'X25519');
    ...
    const existingVm = didDoc.verificationMethod.find((vm: any) => vm.id === keyId);
