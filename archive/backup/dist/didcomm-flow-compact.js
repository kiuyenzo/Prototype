 ...
 */
export async function initiateRequest(agent, peerDID, serviceRequest) {
    ...
    await packAndSend(agent, 'https://didcomm.org/present-proof/3.0/request-presentation', myDID, peerDID, {
        presentation: vp,
        serviceRequest
    }, sessionId);
 ...
 */
export async function handlePeerPresentation(agent, message) {
    const myDID = await getMyDID(agent);
    ...
    // Sende VP_A zurück
    await packAndSend(agent, 'https://didcomm.org/present-proof/3.0/presentation', myDID, message.from, { presentation: vpA }, `${sessionId}-response`, sessionId);
    console.log('[NF_A] Sent VP_A');
}
/**
 * Phase 3: NF_A empfängt Authorized
 */
export async function handleAuthorized(message) {
 ...
 */
export async function sendServiceRequest(agent, sessionId, request) {
    const session = sessions.get(sessionId);
    if (!session || session.status !== 'authorized') {
        throw new Error('Session not authorized');
    }
    const myDID = await getMyDID(agent);
    await packAndSend(agent, 'https://example.com/nf-service-request', myDID, session.peer, request, `${sessionId}-service-${Date.now()}`, sessionId);
 ...
 */
export async function handleInitialRequest(agent, message) {
    const myDID = await getMyDID(agent);
    ...
    // Sende VP_B zurück
    await packAndSend(agent, 'https://didcomm.org/present-proof/3.0/request-presentation', myDID, message.from, {
        presentation: vpB,
        needsYourVP: true // Signal: Ich brauche auch dein VP
    }, `${sessionId}-mutual`, sessionId);
    console.log('[NF_B] Sent VP_B, waiting for VP_A');
}
/**
 * Phase 2: NF_B empfängt VP_A von NF_A
 */
export async function handleFinalPresentation(agent, message) {
    const myDID = await getMyDID(agent);
    const sessionId = message.thid;
    const session = sessions.get(sessionId);
    if (!session)
    ...
    await packAndSend(agent, 'https://didcomm.org/present-proof/3.0/ack', myDID, message.from, {
        status: 'authorized',
 ...
 */
export async function handleServiceRequest(agent, message, businessLogic) {
    const myDID = await getMyDID(agent);
    const sessionId = message.thid;
    const session = sessions.get(sessionId);
    if (!session || session.status !== 'authorized') {
    ...
    // Send response
    await packAndSend(agent, 'https://example.com/nf-service-response', myDID, message.from, response, `${message.id}-response`, sessionId);
    console.log('[NF_B] Service response sent');
}
// ============================================================================
// Unified Message Router (für beide NFs)
// ============================================================================
export async function handleDIDCommMessage(agent, packedMessage, role, businessLogic) {
    // Unpack message
...
export function cleanupSessions(maxAge = 3600000) {
    const now = Date.now();
