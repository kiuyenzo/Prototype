 ...
 */
export function selectCredentialsForPD(presentationDefinition, verifiableCredentials) {
    ...
    try {
        ...
        const selectResults = pex.selectFrom(presentationDefinition, verifiableCredentials);
 ...
 */
export function createVPFromSelectedCredentials(presentationDefinition, selectedCredentials, holderDID) {
    ...
    try {
        // Create VP using PEX
        const vp = pex.presentationFrom(presentationDefinition, selectedCredentials, {
            holderDID: holderDID
        });
 ...
 */
export function evaluateVPAgainstPD(presentationDefinition, verifiablePresentation) {
    console.log('🔍 Evaluating VP against Presentation Definition');
    console.log(`   PD: ${presentationDefinition.id}`);
    try {
        // Evaluate VP using PEX
        const evaluationResults = pex.evaluatePresentation(presentationDefinition, verifiablePresentation);
 ...
 */
export function createVPFromPD(presentationDefinition, availableCredentials, holderDID) {
    console.log('📋 Complete PEX flow: Select + Create VP');
    console.log('='.repeat(60));
    // Step 1: Select matching credentials
    const selectResults = selectCredentialsForPD(presentationDefinition, availableCredentials);
    ...
    const vp = createVPFromSelectedCredentials(presentationDefinition, selectedCredentials, holderDID);
    console.log('='.repeat(60));
 ...
 */
export function verifyVPAgainstPD(presentationDefinition, verifiablePresentation) {
    console.log('🔐 Complete PEX verification');
    console.log('='.repeat(60));
    try {
        const evaluationResults = evaluateVPAgainstPD(presentationDefinition, verifiablePresentation);
        console.log('='.repeat(60));
        if (evaluationResults.value) {
 ...
 */
export function validatePresentationDefinition(presentationDefinition) {
