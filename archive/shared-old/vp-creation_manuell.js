...
// DID resolver for explicit resolution (same as Veramo uses internally)
const didResolver = new did_resolver_1.Resolver({
    ...(0, web_did_resolver_1.getResolver)()
});
/**
 * Resolve DID using Veramo's web-did-resolver with explicit logging
 * This shows the DID resolution that Veramo does internally during VP verification
 */
async function resolveDidWithLogging(did) {
    if (!did.startsWith('did:web:')) {
        console.log(`⚠️  Not a did:web DID: ${did}`);
        return null;
    }
    // Convert did:web to URL for display
    const parts = did.replace('did:web:', '').split(':');
    const domain = parts[0];
    const path = parts.slice(1).join('/');
    const url = `https://${domain}/${path}/did.json`;
    console.log(`🌐 Resolving DID (did:web via Veramo resolver)`);
    console.log(`   DID: ${did}`);
    console.log(`   URL: ${url}`);
    const startTime = Date.now();
    try {
        // Use Veramo's web-did-resolver (same resolver used in agent)
        const result = await didResolver.resolve(did);
        const elapsed = Date.now() - startTime;
        if (result.didDocument) {
            console.log(`   ✅ DID Document resolved from GitHub (${elapsed}ms)`);
            return result.didDocument;
        }
        else {
            console.log(`   ⚠️  DID resolution failed: ${result.didResolutionMetadata?.error}`);
            return null;
        }
    }
    catch (err) {
        console.log(`   ⚠️  DID resolution error: ${err.message}`);
        return null;
    }
}
/**
...
async function verifyVerifiablePresentation(agent, presentation) {
    try {
        ...
        // Extract holder DID and resolve it explicitly for visibility
        const holderDid = presentation.holder || presentation.proof?.verificationMethod?.split('#')[0];
        if (holderDid) {
            console.log(`   Holder: ${holderDid}`);
            await resolveDidWithLogging(holderDid);
        }
        // Extract issuer DIDs from credentials and resolve them
        ...
        for (const cred of credentials) {
            ...
            if (issuerDid) {
                ...
                await resolveDidWithLogging(issuerDid);
            }
        }
