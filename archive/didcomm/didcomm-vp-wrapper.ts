...
import {
  ...
  createVPFromPD,
  verifyVPAgainstPD
} from '../shared/vp-creation_manuell.js';
import {
  DIDCOMM_MESSAGE_TYPES,
  createVPAuthRequest,
  createVPWithPD,
  createVPResponse,
  createAuthConfirmation,
...
} from '../shared/didcomm-messages.js';
...
export interface MessageHandlers {
  onVPAuthRequest?: (message: VPAuthRequestMessage) => Promise<void>;
  onVPWithPD?: (message: VPWithPDMessage) => Promise<void>;
  onVPResponse?: (message: VPResponseMessage) => Promise<void>;
  onAuthConfirmation?: (message: AuthConfirmationMessage) => Promise<void>;
  onError?: (error: Error, message?: DIDCommVPMessage) => Promise<void>;
}

/**
 * DIDComm VP Exchange Context
 * Stores state during the multi-step authentication flow
 */
export interface VPExchangeContext {
  ourDid: string;
  theirDid: string;
  ourPresentationDefinition: PresentationDefinition;
  theirPresentationDefinition?: PresentationDefinition;
  ourVP?: any;
  theirVP?: any;
  sessionToken?: string;
  authenticated: boolean;
  messageLog: DIDCommVPMessage[];
}
 ...
 */
export class DIDCommVPWrapper {
  private agent: Agent;
  private messageQueue: DIDCommVPMessage[] = [];
  private contexts: Map<string, VPExchangeContext> = new Map();
   ...
   */
  async initiateVPAuthRequest(
    ourDid: string,
    theirDid: string,
    presentationDefinition: PresentationDefinition
  ): Promise<VPAuthRequestMessage> {
    ...
    const context: VPExchangeContext = {
      ourDid,
      theirDid,
      ourPresentationDefinition: presentationDefinition,
      authenticated: false,
    ...
    const message = createVPAuthRequest(
      ourDid,
      theirDid,
      presentationDefinition,
      'Please provide a Verifiable Presentation matching this definition'
    );
   ...
   */
  async handleVPAuthRequest(
    message: VPAuthRequestMessage,
    ourDid: string,
    credentials: any[],
    ourPresentationDefinition: PresentationDefinition
  ): Promise<VPWithPDMessage> {
    ...
    try {
      ...
      const vp = await createVPFromPD(
        this.agent,
        ourDid,
        credentials,
        message.body.presentation_definition
      );
      ...
      const errorMessage = createErrorMessage(
        ...
        message.from,
        'vp_creation_failed',
        `Failed to create VP: ${error.message}`
      );
   ...
   */
  async handleVPWithPD(
    message: VPWithPDMessage,
    credentials: any[],
    ourDid?: string,
    ourPresentationDefinition?: any
  ): Promise<VPResponseMessage> {
    ...
    try {
      ...
      const verificationResult = await verifyVPAgainstPD(
        this.agent,
        message.body.verifiable_presentation,
        context.ourPresentationDefinition
      );
      ...
      const ourVP = await createVPFromPD(
        this.agent,
        context.ourDid,
        credentials,
        message.body.presentation_definition
      ...
      const responseMessage = createVPResponse(
        context.ourDid,
        message.from,
        ourVP,
        'Here is my VP matching your PD'
      );
      ...
      const errorMessage = createErrorMessage(
        ...
        `Failed to verify or create VP: ${error.message}`
      );
      this.messageQueue.push(errorMessage);
      throw error;
    }
  ...
  async handleVPResponse(
    message: VPResponseMessage
  ): Promise<AuthConfirmationMessage> {
    ...
    try {
      ...
      // Send rejection
      const rejectionMessage = createAuthConfirmation(
        context.ourDid,
        message.from,
        'REJECTED',
        undefined,
        `VP verification failed: ${error.message}`
      );
   ...
   */
  async handleAuthConfirmation(
    message: AuthConfirmationMessage
  ): Promise<void> {
  ...
  async sendServiceRequest(
    ...
    const requestMessage = createServiceRequest(
      context.ourDid,
      theirDid,
      service,
      action,
   ...
   */
  getContext(did: string): VPExchangeContext | undefined {
   ...
   */
  isAuthenticated(did: string): boolean {
   ...
   */
  getPendingMessages(): DIDCommVPMessage[] {
   ...
   */
  private generateSessionToken(context: VPExchangeContext): string {
    const tokenData = {
...
export interface MutualAuthResult {
  nfAAuthenticated: boolean;
  nfBAuthenticated: boolean;
  sessionToken?: string;
  error?: string;
}
 ...
 */
export async function performMutualAuthentication(
  agentA: Agent,
  agentB: Agent,
  didA: string,
  didB: string,
  ...
  try {
    ...
    console.log('================================================================================');
    const vpWithPD = await wrapperB.handleVPAuthRequest(
      authRequest,
      didB,
      credentialsB,
      pdB
