...
import {
  DIDCommVPWrapper,
  VPExchangeContext
} from '../shared/didcomm-vp-wrapper.js';
import {
  DIDCommVPMessage,
...
} from '../shared/presentation-definitions.js';
...
import {
  ...
  isEncryptedMessage,
  verifyEncryption
} from '../shared/didcomm-encryption.js';
import { precacheDIDs } from '../shared/did-resolver-cache.js';
...
const pendingServiceRequests: Map<string, { service: string; action: string; params?: any; timestamp: number }> = new Map();
 ...
 */
async function handleServiceRequest(request: ServiceRequestMessage): Promise<ServiceResponseMessage> {
  ...
  try {
    switch (message.type) {
      ...
      case DIDCOMM_MESSAGE_TYPES.SERVICE_REQUEST:
        ...
        const serviceReq = message as ServiceRequestMessage;
      ...
      case DIDCOMM_MESSAGE_TYPES.SERVICE_RESPONSE:
        ...
        const serviceResp = message as ServiceResponseMessage;
    ...
    if (req.url === '/test/gateway-visibility' && req.method === 'POST') {
      ...
      req.on('end', async () => {
        try {
          ...
          const packedMessage = await packDIDCommMessage(agent, testMessage, target, MY_DID);

          // Analyze what the gateway sees
          let gatewayCanRead = true;
          let format = 'unknown';
          let visibleSecrets: string[] = [];
