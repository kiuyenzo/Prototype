...
import { KeyStore, DIDStore, DataStore, DataStoreORM, PrivateKeyStore } from '@veramo/data-store';
...
import { DIDCommVPWrapper } from './src/didcomm-vp-wrapper.js';
import {
  DIDCOMM_MESSAGE_TYPES,
  createVPAuthRequest,
  createServiceRequest,
  createServiceResponse
} from './src/didcomm-messages.js';
...
const pendingServiceRequests = new Map<string, { service: string; action: string; params: any; timestamp: number }>();
...
async function loadCredentials(): Promise<any[]> {
  ...
  return credentials
    .filter((cred: any) => cred.verifiableCredential.type.includes('NetworkFunctionCredential'))
    .map((cred: any) => cred.verifiableCredential);
}

/**
 * Call NF container for business logic (localhost:3000)
 */
async function callNFService(service: string, action: string, params: any): Promise<any> {
 ...
 */
async function sendResponseToNF(data: any): Promise<void> {
 ...
 */
async function sendDIDCommMessage(message: any, targetDid: string): Promise<void> {
  ...
  const packedMessage = await packDIDCommMessage(agent, message, targetDid, MY_DID);

  // Only verify JWE format for encrypted mode
  if (PACKING_MODE === 'encrypted') {
    verifyEncryption(packedMessage);
  }
 ...
 */
async function handleIncomingMessage(messageOrEncrypted: any): Promise<any> {
  ...
  switch (message.type) {
    ...
    case DIDCOMM_MESSAGE_TYPES.VP_RESPONSE: {
      ...
      if (!session2) throw new Error('No active session found');
    ...
    case DIDCOMM_MESSAGE_TYPES.AUTH_CONFIRMATION: {
      ...
      if (pendingRequest) {
        ...
        const serviceRequest = createServiceRequest(
          MY_DID, senderDid,
          pendingRequest.service, pendingRequest.action, pendingRequest.params
        );
    ...
    case DIDCOMM_MESSAGE_TYPES.SERVICE_REQUEST: {
      ...
      if (!serviceSession || serviceSession.status !== 'authenticated') {
        ...
        return createServiceResponse(MY_DID, serviceReq.from, 'error', undefined,
          'Not authenticated. Please complete VP exchange first.');
      }
      ...
      try {
        const nfResponse = await callNFService(
          serviceReq.body.service,
          serviceReq.body.action,
          serviceReq.body.params
        );
        return createServiceResponse(MY_DID, serviceReq.from, 'success', nfResponse);
      } catch (error: any) {
        return createServiceResponse(MY_DID, serviceReq.from, 'error', undefined, error.message);
      }
    }

    case DIDCOMM_MESSAGE_TYPES.SERVICE_RESPONSE: {
      ...
      req.on('end', async () => {
        try {
          ...
          if (!session || session.status !== 'authenticated') {
            ...
            const authRequest = createVPAuthRequest(MY_DID, target, MY_PD,
          ...
          const serviceRequest = createServiceRequest(MY_DID, target, service, action, params);
