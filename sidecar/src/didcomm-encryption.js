...
async function packDIDCommMessage(agent, message, recipientDid, senderDid) {
    ...
    try {
        ...
        // Veramo's DIDComm plugin handles DID resolution internally
        if (packingMode === 'authcrypt' || packingMode === 'anoncrypt') {
            console.log(`🌐 Resolving recipient DID via Veramo...`);
        }

        // Pack the message using Veramo's DIDComm plugin (resolves DID automatically)
