...
kubectl config use-context kind-cluster-a > /dev/null 2>&1
kubectl exec -n nf-a-namespace $NF_A_POD -c veramo-sidecar -- node /app/shared/create-nf-credentials.js cluster-a > /dev/null 2>&1
kubectl config use-context kind-cluster-b > /dev/null 2>&1
kubectl exec -n nf-b-namespace $NF_B_POD -c veramo-sidecar -- node /app/shared/create-nf-credentials.js cluster-b > /dev/null 2>&1
