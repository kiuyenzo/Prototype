...
# Check if istioctl is installed (prefer global install via: brew install istioctl)
if command -v istioctl &> /dev/null; then
    echo "✅ Using global istioctl: $(istioctl version --client --short 2>/dev/null || echo 'installed')"
else
    echo "📥 Downloading Istio $ISTIO_VERSION (install globally with: brew install istioctl)"
...
echo "⏭️  Skipped - use ./scripts/deploy/build-and-deploy.sh for sidecar architecture"
echo ""
...
# Update deploy/cluster-a/gateway.yaml with Cluster-B's IP and port
echo "🔄 Updating deploy/cluster-a/gateway.yaml..."
sed -i.bak "s|addresses:.*# Docker network IP of Cluster-B|addresses:\n  - $CLUSTER_B_IP  # Docker network IP of Cluster-B|" deploy/cluster-a/gateway.yaml
sed -i.bak "s|address: 172\.[0-9]*\.[0-9]*\.[0-9]*$|address: $CLUSTER_B_IP|g" deploy/cluster-a/gateway.yaml
sed -i.bak "s|number: [0-9]*  # HTTPS port|number: $CLUSTER_B_HTTPS_NODEPORT  # HTTPS port|g" deploy/cluster-a/gateway.yaml
sed -i.bak "s|https-didcomm: [0-9]*$|https-didcomm: $CLUSTER_B_HTTPS_NODEPORT|g" deploy/cluster-a/gateway.yaml

# Update deploy/cluster-b/gateway.yaml with Cluster-A's IP and port
echo "🔄 Updating deploy/cluster-b/gateway.yaml..."
sed -i.bak "s|addresses:.*# Docker network IP of Cluster-A|addresses:\n  - $CLUSTER_A_IP  # Docker network IP of Cluster-A|" deploy/cluster-b/gateway.yaml
sed -i.bak "s|address: 172\.[0-9]*\.[0-9]*\.[0-9]*$|address: $CLUSTER_A_IP|g" deploy/cluster-b/gateway.yaml
sed -i.bak "s|number: [0-9]*  # HTTPS port|number: $CLUSTER_A_HTTPS_NODEPORT  # HTTPS port|g" deploy/cluster-b/gateway.yaml
sed -i.bak "s|https-didcomm: [0-9]*$|https-didcomm: $CLUSTER_A_HTTPS_NODEPORT|g" deploy/cluster-b/gateway.yaml

# Clean up backup files
rm -f deploy/cluster-a/gateway.yaml.bak deploy/cluster-b/gateway.yaml.bak
...
kubectl config use-context kind-cluster-a
kubectl apply -f deploy/cluster-a/01-namespace.yaml
...
kubectl config use-context kind-cluster-b
kubectl apply -f deploy/cluster-b/01-namespace.yaml
...
kubectl config use-context kind-cluster-a
kubectl exec -n nf-a-namespace $POD_A -c veramo-sidecar -- node /app/scripts/setup/create-credentials.mjs 2>&1 | grep -E "(✅|Created|successfully)" || true

kubectl config use-context kind-cluster-b
kubectl exec -n nf-b-namespace $POD_B -c veramo-sidecar -- node /app/scripts/setup/create-credentials.mjs 2>&1 | grep -E "(✅|Created|successfully)" || true
